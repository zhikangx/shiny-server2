...
server <- function(input, output) {
        ...
        output$map <- renderLeaflet({
                ...
                target_name <- as.character(input$caption)
                ## Set Target Address
                # target_name <- "北京市朝阳区十里堡朝阳路都会国际"
                # target_name <- as.character(input$caption)
                
                target_address <- getGeocoding(address = target_name, city = '北京', ak = 'ZxjZ6VnwlxvRarbnVtg38IcCwpiGLZzN')
                ...
                if (1==1) {
                        ...
                        find_geo_location <- function(target_list) {
                                ...
                                for (i in 1:nrow(target)) {
                                        # i=1
                                        tem_address <- getGeocoding(address = target$address[i], city = '北京', ak = 'ZxjZ6VnwlxvRarbnVtg38IcCwpiGLZzN')
                ...
                if (1==1) {
                        ...
                        db_nearby_amenty <- rbind(nearby_mdc_store, nearby_station, nearby_stb_store, nearby_restaurant)
                ...
                if (1==1) {
                        ...
                        mixpace_geo_bj <- read.csv("mixpace_geo_bj.csv",stringsAsFactors = FALSE)
                        landmark_geo_bj <- read.csv("landmark_geo_bj.csv",stringsAsFactors = FALSE)
                        df_competor_bj <- read.csv("db_compete_bj.csv",stringsAsFactors = FALSE)
                }
                ...
                m = addLayersControl(m,
                                     ...
                                     overlayGroups = c("1. nearby station", "3. nearby Mcdonalds", "4. nearby Starbucks", "5. Landmark", "6. nearby Restaurant", "X1 - Mixpace(500m)", "X2 - Mixpace(seat)", "X3 - All Competitors(500m)", "X4 - All Competitors(seat)")
                ...
                m = addCircles(m, lng = mixpace_geo_bj$lng, lat = mixpace_geo_bj$lat, weight = 1,color = "blue", radius = 500, label = mixpace_geo_bj$name, labelOptions = labelOptions(noHide = F, direction = 'bottom',offset=c(0,15)), group = "X1 - Mixpace(500m)")
                m <- addAwesomeMarkers(m, lng = landmark_geo_bj$lng, lat = landmark_geo_bj$lat, label=landmark_geo_bj$name,labelOptions = labelOptions(noHide = F, direction = 'bottom', offset=c(0,15)),icon = icon.landmark, group = ("5. Landmark"))
                ...
                m<- m %>% addTiles() %>%
                        ...
                        addEasyButton(easyButton(
                                ...
                                onClick = JS("function(btn, map){ map.setView([39.92114,116.5086],12);}")))
                
                ### hide layer 
                m <- m %>% hideGroup(c("5. Landmark", "6. nearby Restaurant", "X1 - Mixpace(500m)", "X2 - Mixpace(seat)", "X3 - All Competitors(500m)", "X4 - All Competitors(seat)"))
                ### Show the Map
                m
        })
